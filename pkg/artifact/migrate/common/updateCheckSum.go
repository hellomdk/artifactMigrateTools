...
func UpdateArtifactSum(arti config.Arti) {
	...
	got := jf.UpdateArtifactCheckSum(arti.Repo, arti.Path, arti.Sha1, arti.Sha256, arti.Md5)
	fmt.Println(got)
}

func UpdateArtifactProp(repoKey, repoPath, properties string) {
	config, er := config.NewConfig()
	if er != nil {
		fmt.Println(config)
	}
	...
	got := jf.UpdateArtifactProperties(repoKey, repoPath, properties)
	fmt.Println(got)
}
